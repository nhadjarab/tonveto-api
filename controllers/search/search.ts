...
export const advancedSearch = async (
  ...
  try {
    ...
    const filteredClinics = clinics.filter(
      (clinic) =>
        clinic.name.toLowerCase().includes((clinic_name as string)?.toLowerCase()) ||
        clinic.city.toLowerCase().includes((city as string)?.toLowerCase()) ||
        clinic.address.toLowerCase().includes((address as string)?.toLowerCase()) ||
        clinic.zip_code.toLowerCase().includes((zip_code as string)?.toLowerCase()) ||
        clinic.country.toLowerCase().includes((country as string)?.toLowerCase())
    );

    const filteredVets = vets.filter(
      (vet) =>
        vet.first_name.toLowerCase().includes((vet_name as string)?.toLowerCase()) ||
        vet.last_name.toLowerCase().includes((vet_name as string)?.toLowerCase()) ||
        (vet.first_name + " " + vet.last_name)
          .toLowerCase()
          .includes((vet_name as string)?.toLowerCase()) ||
        vet.specialities.some((speciality) =>
          speciality.name.toLowerCase().includes((specialty as string)?.toLowerCase())
        )
    );
